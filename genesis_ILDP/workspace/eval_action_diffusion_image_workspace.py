...
class EvalActionDiffusionImageWorkspace(BaseWorkspace):
    ...
    This workspace loads a checkpoint and uses its config for model/policy/dataset,
    while allowing runtime overrides for evaluation-specific parameters.

    The eval config (cfg) should only contain:
    - checkpoint_path: path to trained model
    - device: evaluation device override
    - output_dir: where to save results
    - env_runner: runtime parameter overrides
    - logging: wandb settings

    All model/policy/dataset settings come from the checkpoint automatically.
    """
