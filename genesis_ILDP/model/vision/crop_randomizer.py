...
class CropRandomizer(nn.Module):
    ...
    def forward_in(self, inputs, generator=None):
        """
    ...
    def forward(self, inputs, generator=None):
        return self.forward_in(inputs, generator=generator)
...
def sample_random_image_crops(images, crop_height, crop_width, num_crops, pos_enc=False, generator=None):
    """
    ...
    Args:
        ...
        pos_enc (bool): if True, also add 2 channels for spatial encoding
        generator (torch.Generator): optional generator for reproducible sampling
    ...
    crop_inds_h = (max_sample_h * torch.rand(*source_im.shape[:-3], num_crops, device=device, generator=generator)).long()
    crop_inds_w = (max_sample_w * torch.rand(*source_im.shape[:-3], num_crops, device=device, generator=generator)).long()
