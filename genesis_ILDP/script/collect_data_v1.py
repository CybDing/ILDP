...
def robot_to_display(robot_x, robot_y):
    """Convert robot coordinates to pygame display coordinates.
    Robot: x is negative, y is positive
...
def display_to_robot(screen_x, screen_y):
    """Convert pygame display coordinates to robot coordinates.
...
latest_data = {
    "cur_keypoints": [],
    "target_keypoints": [],
    "agent_pos": [WORKSPACE_X_MIN, WORKSPACE_Y_MIN],
...
class DataCollector:
    ...
    def record_timestep(self, obs_data):
        ...
        current_agent_pos = np.array(obs_data.get('agent_pos', [WORKSPACE_X_MIN, WORKSPACE_Y_MIN])[:2], dtype=np.float32)
        target_pos = np.array(obs_data.get('target_pos', [WORKSPACE_X_MIN, WORKSPACE_Y_MIN])[:2], dtype=np.float32)
...
def save_buffer(obs_data, mouse_pos, timestamp):
    ...
    with data_lock:  # Thread safety
        ...
        # Convert mouse position to robot coordinates (returns negative x)
        target_pos = list(display_to_robot(mouse_pos[0], mouse_pos[1]))
...
class SimpleT:
    ...
    def update(self, keypoints):
        if len(keypoints) >= 8:
            self.points = [robot_to_display(p[0], p[1]) for p in keypoints]
...
class SimpleEEF:
    ...
    def update(self, agent_pos):
        if len(agent_pos) >= 2:
            self.pos = robot_to_display(agent_pos[0], agent_pos[1])
...
while running:
    ...
    if action_sending and (current_time - last_action_time) > (1.0 / action_frequency):
        ...
        robot_x, robot_y = display_to_robot(current_mouse_pos[0], current_mouse_pos[1])
        # Clamp to workspace bounds
        action_x = max(WORKSPACE_X_MIN, min(WORKSPACE_X_MAX, robot_x))
        action_y = max(WORKSPACE_Y_MIN, min(WORKSPACE_Y_MAX, robot_y))
